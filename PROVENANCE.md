...
|---|---|
| `detect_live.py` (`StopSign.py`) | ✅ **Recovered verbatim from the Pi's SD card.** The card was pulled and read directly via a partition-recovery tool (DiskGenius) reading the ext4 rootfs partition, since it isn't natively mounted by Windows. Path on the Pi: `/home/km499/yolo_tf_project/StopSign_Dataset/StopSign.py`. An earlier reconstruction attempt (built from chat logs and terminal output alone, before the card was recovered) got every numeric parameter right — threshold, input size, serial format, reset delay — because those were independently confirmed by terminal output. It missed the file's actual structure: a full CLI (`argparse`, `--input`/`--display` flags) and a batch/directory-processing mode, neither of which appeared anywhere in the chat logs. Superseded reconstruction retained at `experiments/detect_live_v1_reconstructed.py` for the record. |
